...
char confFile[MAX_PATH]="C:\\allview_agentd.conf";
char logFile[MAX_PATH]="{EventLog}";
WORD confListenPort=10050; // Allview team: New defailt port 10000 -> 10050
...
static BOOL LoadSubAgent(char *name,char *cmdLine)
{
   ...
   sbi->init=(int (__allview_api *)(char *,SUBAGENT_COMMAND **))GetProcAddress(sbi->hModule,"allview_subagent_init");
   sbi->shutdown=(void (__allview_api *)(void))GetProcAddress(sbi->hModule,"allview_subagent_shutdown");
...
void Main(void)
{
   ...
   if (IsStandalone())
   {
      ...
      printf("\n*** Allview Win32 agent operational. Press ESC to terminate. ***\n");
      while(1)
      {
         ch=getch();
         if (ch==0)
...
int main(int argc,char *argv[])
{
   ...
   else
   {
      if (!Initialize())
      {
         printf("Allview Win32 agent initialization failed\n");
         return 1;
      }
      Main();
   }
