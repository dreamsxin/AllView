...
AllviewW32 is Allview agent for Win32 systems. It will work on Windows NT 4.0,
Windows 2000, Windows XP and Windows Server 2003. AllviewW32 doesn't supposed
...
1. Unpack AllviewW32.exe
2. Create configuration file c:\allview_agentd.conf (it has the same syntax as
   for UNIX agent).
3. Run command "AllviewW32.exe install" to install Allview agent as service.
   If you wish to use configuration file other that c:\allview_agentd.conf,
   you should use the following command for service installation:
   "AllviewW32.exe --config <your_configuration_file> install". Full path to
   configuration file should be specified.

Now you can use Control Panel to start agent's service or run
"AllviewW32.exe start".

Windows NT 4.0 Note:
AllviewW32 uses PDH (Performance Data Helper) API to gather various system
...
Usage: allvieww32 [options] [command]
...
Allview Win32 agent suports the following configuration parameters:

Server = <ip_address>[,<ip_address>[,<ip_address> ...]]
  Sets IP address(es) of Allview server(s). Agent will accept connections only
...
Parameters supported by Allview Win32 Agent
------------------------------------------

I. Allview standard parameters
