...
//
// Handler for parameters which always returns string constant (like version[allview_agent])
//
...
static AGENT_COMMAND commands[]=
{
   ...
   { "version[allview_agent]",NULL,H_StringConstant,AGENT_VERSION },
   { "",NULL,NULL,NULL }
};
...
void ProcessCommand(char *received_cmd,char *result)
{
   ...
   char *strResult=NULL,cmd[MAX_ALLVIEW_CMD_LEN];
...
finish_cmd_processing:;
   switch(iRC)
   {
      ...
      case SYSINFO_RC_NOTSUPPORTED:
         strcpy(result,"ALV_NOTSUPPORTED\n");
         statUnsupportedRequests++;
         break;
      case SYSINFO_RC_ERROR:
         strcpy(result,"ALV_ERROR\n");
         statFailedRequests++;
         break;
      default:
         strcpy(result,"ALV_ERROR\n");
