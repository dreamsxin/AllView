	...
	if(sucker_num == 0)
	{
		if(pids != NULL)
		{
			for(i=0;i<CONFIG_SUCKERD_FORKS+CONFIG_TRAPPERD_FORKS-1;i++)
			{
				...
				{
					allview_log( LOG_LEVEL_WARNING, "Cannot kill process. PID=[%d] [%s]", pids[i], strerror(errno));
				}
			}
		}
		...
		{
			allview_log( LOG_LEVEL_WARNING, "Cannot remove PID file [%s] [%s]",
...
void	signal_handler( int sig )
{
	if( SIGALRM == sig )
	{
		...
		allview_log( LOG_LEVEL_DEBUG, "Timeout while executing operation." );
	}
	else if( SIGQUIT == sig || SIGINT == sig || SIGTERM == sig || SIGPIPE == sig )
	{
		allview_log( LOG_LEVEL_ERR, "Got QUIT or INT or TERM or PIPE signal. Exiting..." );
		uninit();
		exit( FAIL );
	}
...
/*	else if( SIGCHLD == sig )
	{
		allview_log( LOG_LEVEL_ERR, "One child died. Exiting ..." );
		uninit();
		exit( FAIL );
	}*/
	else if( SIGPIPE == sig)
...
void	daemon_init(void)
{
	...
	if((getuid()==0) || (getgid()==0))
	{
		...
		{
			fprintf(stderr,"Cannot setgid or setuid to allview [%s]", strerror(errno));
			exit(FAIL);
		}
		...
		{
			fprintf(stderr,"Cannot setegid to allview [%s]\n", strerror(errno));
			exit(FAIL);
		}
		if( seteuid(pwd->pw_uid) ==-1)
		{
			fprintf(stderr,"Cannot seteuid to allview [%s]\n", strerror(errno));
			exit(FAIL);
		}
#endif
...
{
	printf("allview_server - AllView server process v1.0 release\n");
	printf("Usage: %s [-h] [-c <file>]\n", prog);
	printf("\nOptions:\n");
	printf("  -c <file>   Specify configuration file. Default is /etc/allview/allview_server.conf\n");
	printf("  -h          Help\n");
	exit(-1);
}
...
/*		 PARAMETER	,VAR	,FUNC,	TYPE(0i,1s),MANDATORY,MIN,MAX	*/
	...
	if(CONFIG_FILE == NULL)
	{
		CONFIG_FILE=strdup("/etc/allview/allview_server.conf");
	}
	...
	{
		allview_log( LOG_LEVEL_CRIT, "DBName not in config file");
		exit(1);
	}
	if(CONFIG_PID_FILE == NULL)
	{
		CONFIG_PID_FILE=strdup("/tmp/allview_server.pid");
	}
	if(CONFIG_ALERT_SCRIPTS_PATH == NULL)
	{
		CONFIG_ALERT_SCRIPTS_PATH=strdup("/home/allview/bin");
	}
...
int	get_value(double *result,char *result_str,DB_ITEM *item, char *error, int max_error_len)
{
	...
	if(item->type == ITEM_TYPE_ALLVIEW)
	{
...
#else
		allview_log(LOG_LEVEL_WARNING, "Support of SNMP parameters was no compiled in");
		allview_syslog("Support of SNMP parameters was no compiled in. Cannot process [%s:%s]", item->host, item->key);
		res=NOTSUPPORTED;
#endif
	}
	...
	{
		allview_log(LOG_LEVEL_WARNING, "Not supported item type:%d",item->type);
		allview_syslog("Not supported item type:%d",item->type);
		res=NOTSUPPORTED;
	}
	alarm(0);
	return res;
...
/* Host status	0 == MONITORED
	...
	{
		allview_log(LOG_LEVEL_DEBUG, "No items to update for minnextcheck.");
		res = FAIL; 
	}
	else
	{
...
void update_key_status_thread(MYSQL *database, int hostid,int host_status)
{
	...
	allview_log(LOG_LEVEL_DEBUG, "In update_key_status()");
	...
	if( DBnum_rows(result) == 0)
	{
		allview_log( LOG_LEVEL_DEBUG, "No items to update.");
	}
	else
	{
...
int get_values(void)
{
	...
	for(i=0;i<DBnum_rows(result);i++)
	{
		...
		allview_log( LOG_LEVEL_DEBUG, "GOT VALUE [%s]", value_str );
...
/*			if(HOST_STATUS_UNREACHABLE == item.host_status)*/
			if(HOST_AVAILABLE_TRUE != item.host_available)
			{
				...
				allview_log( LOG_LEVEL_WARNING, "Enabling host [%s]", item.host );
				allview_syslog("Enabling host [%s]", item.host );
...
/* Why this break??? Trigger needs to be updated anyway!
		...
		{
			allview_log( LOG_LEVEL_WARNING, "Parameter [%s] is not supported by agent on host [%s]", item.key, item.host );
			allview_syslog("Parameter [%s] is not supported by agent on host [%s]", item.key, item.host );
...
/*			if(HOST_STATUS_UNREACHABLE == item.host_status)*/
		...
		else if(res == NETWORK_ERROR)
		{
			...
			{
				allview_log( LOG_LEVEL_WARNING, "Host [%s] will be checked after [%d] seconds", item.host, DELAY_ON_NETWORK_FAILURE );
				allview_syslog("Host [%s] will be checked after [%d] seconds", item.host, DELAY_ON_NETWORK_FAILURE );
...
/* Possibly, other logic required? */
		...
		{
			allview_log( LOG_LEVEL_WARNING, "Getting value of [%s] from host [%s] failed (ALV_ERROR)", item.key, item.host );
			allview_syslog("Getting value of [%s] from host [%s] failed (ALV_ERROR)", item.key, item.host );
			allview_log( LOG_LEVEL_WARNING, "The value is not stored in database.");

			break;
		}
		else
		{
			allview_log( LOG_LEVEL_WARNING, "Getting value of [%s] from host [%s] failed", item.key, item.host );
			allview_syslog("Getting value of [%s] from host [%s] failed", item.key, item.host );
			allview_log( LOG_LEVEL_WARNING, "The value is not stored in database.");
		}
	}
...
void *main_poller_loop()
{
	...
	allview_log( LOG_LEVEL_DEBUG, "In main_poller_loop()");
	for(;;)
	{
//		allview_log( LOG_LEVEL_WARNING, "poller: waiting for state 1..%d [poll values from agents] State [%d]", (CONFIG_SUCKERD_FORKS-4)-1, state);
		allview_log( LOG_LEVEL_WARNING, "poller: waiting for state 1 [poll values from agents] State [%d]", state);
...
//					allview_log( LOG_LEVEL_WARNING, "poller: looking for status [0] host [%s] key [%s] status [%d]", DBget_field(shared_result,i,2), DBget_field(shared_result,i,1), requests[i].status);
				num = i;
				if(requests[i].status == 0)
				{
//					allview_log( LOG_LEVEL_WARNING, "poller: itemid [%d]", atoi(DBget_field(shared_result,i,0)) );
					if(requests[i].status!=0)
						allview_log( LOG_LEVEL_WARNING, "poller: ERROR status [%d] must be [0] host [%s] key  [%s]", requests[num].status, item.host, item.key);
					requests[i].status = 1;
...
/*			allview_log( LOG_LEVEL_WARNING, "poller: number of already retrieved items [%d] total [%d]", i, request_count);*/

			/* 0 5 6 ? */
//			allview_log( LOG_LEVEL_WARNING, "poller: num [%d] answer count [%d] request count [%d]", num, answer_count, request_count);
//			if(processed < request_count-1)
			if( (num < request_count) && (unprocessed_flag == 1))
			{
				/* do logic */
//				allview_log( LOG_LEVEL_WARNING, "poller: processing host [%s] key [%s] status [%d]", DBget_field(shared_result,num,2), DBget_field(shared_result,num,1), requests[num].status);
				...
				if(requests[num].status!=1)
					allview_log( LOG_LEVEL_WARNING, "poller: ERROR status [%d] must be [1] host [%s] key  [%s]", requests[num].status, item.host, item.key);
				...
				if(requests[num].status!=1)
				{
					allview_log( LOG_LEVEL_WARNING, "poller: ERROR2 status [%d] must be [1] host [%s] key [%s] num [%d] processed [%d] request_count [%d]", requests[num].status, item.host, item.key, num, processed, request_count);
					allview_log( LOG_LEVEL_WARNING, "poller: ERROR2 host2 [%s] item.host [%s]", DBget_field(shared_result,num,2), item.host);
				}
				requests[num].status = 2;
				answer_count++;
//				allview_log( LOG_LEVEL_WARNING, "poller: answer count [%d] request count [%d]", answer_count, request_count);
				pthread_mutex_unlock (&poller_mutex);

//				allview_log( LOG_LEVEL_WARNING, "poller: processed [%d] request count [%d]", processed, request_count);
			}

//			if(num == request_count-1)
			pthread_mutex_lock (&poller_mutex);
//			allview_log( LOG_LEVEL_WARNING, "poller:      num [%d] answer count [%d] request count [%d] processed [%d]", num, answer_count, request_count, processed);
...
/*		if(state == 1)
		{
			state = 2;
			allview_log( LOG_LEVEL_WARNING, "poller: broadcast state [%d]", state);
			pthread_cond_broadcast(&result_cv);
		}
		else
		{
			allview_log( LOG_LEVEL_WARNING, "poller: do not broadcast state [2]");
		}
*/
		state = 2;
		allview_log( LOG_LEVEL_WARNING, "poller: broadcast state [%d]", state);
...
#endif
{
	...
	for(;;)
	{
		...
		allview_log( LOG_LEVEL_DEBUG, "Spent %d seconds while updating values", (int)time(NULL)-now );
...
#endif
		allview_log( LOG_LEVEL_DEBUG, "Nextcheck:%d Time:%d", nextcheck, (int)time(NULL) );
		...
		if(sleeptime>0)
		{
			...
			allview_log( LOG_LEVEL_DEBUG, "Sleeping for %d seconds",
					sleeptime );
#ifdef HAVE_FUNCTION_SETPROCTITLE
		...
		{
			allview_log( LOG_LEVEL_DEBUG, "No sleeping" );
		}
	}
}
...
void	init_threads(void)
{
	...
	for(i=0;i < CONFIG_SUCKERD_FORKS;i++){
		allview_log( LOG_LEVEL_WARNING, "Creating thread %d", i);
		if(i==0)
		{
...
//			rc = pthread_create(&threads[i], NULL, main_housekeeper_loop, (void *)i);
		...
		if (rc){
			allview_log( LOG_LEVEL_ERR, "ERROR from pthread_create() [%m]");
			exit(-1);
		}
	}
	pthread_exit(NULL);
...
void	process_trapper_child(int sockfd)
{
	...
	allview_log( LOG_LEVEL_DEBUG, "Length [%d]", strlen(result));
	allview_log( LOG_LEVEL_DEBUG, "Sockfd [%d]", sockfd);
	if( write(sockfd,result,strlen(result)) == -1)
	{
		allview_log( LOG_LEVEL_WARNING, "Error sending result back [%s]",strerror(errno));
		allview_syslog("Trapper: error sending result back [%s]",strerror(errno));
	}
	allview_log( LOG_LEVEL_DEBUG, "After write()");
	alarm(0);
}
...
int	tcp_listen(const char *host, int port, socklen_t *addrlenp)
{
	...
	{
		allview_log( LOG_LEVEL_CRIT, "Cannot create socket");
		exit(1);
	}
	...
	{
		allview_log( LOG_LEVEL_CRIT, "Cannot bind to port %d. Another allview_trapperd running ?", port);
		exit(1);
	}
...
void	child_trapper_main(int i,int listenfd, int addrlen)
{
	...
	allview_log( LOG_LEVEL_DEBUG, "In child_main()");

/*	allview_log( LOG_LEVEL_WARNING, "allview_trapperd %ld started",(long)getpid());*/
	allview_log( LOG_LEVEL_WARNING, "server #%d started [Trapper]", i);

	allview_log( LOG_LEVEL_DEBUG, "Before DBconnect()");
	DBconnect();
	allview_log( LOG_LEVEL_DEBUG, "After DBconnect()");
...
#endif
		allview_log( LOG_LEVEL_DEBUG, "Before accept()");
		connfd=accept(listenfd,&cliaddr, &clilen);
		allview_log( LOG_LEVEL_DEBUG, "After accept()");
#ifdef HAVE_FUNCTION_SETPROCTITLE
...
/* Parse the command-line. */
	...
	allview_log( LOG_LEVEL_WARNING, "Starting allview_server...");
...
/*	allview_log( LOG_LEVEL_WARNING, "allview_suckerd #%d started",sucker_num);*/
...
/* First instance of allview_suckerd performs housekeeping procedures */
		allview_log( LOG_LEVEL_WARNING, "server #%d started [Housekeeper]",sucker_num);
		main_housekeeper_loop();
	}
	else if(sucker_num == 1)
	{
/* Second instance of allview_suckerd sends alerts to users */
		allview_log( LOG_LEVEL_WARNING, "server #%d started [Alerter]",sucker_num);
		main_alerter_loop();
	}
	else if(sucker_num == 2)
	{
/* Third instance of allview_suckerd periodically re-calculates 'nodata' functions */
		allview_log( LOG_LEVEL_WARNING, "server #%d started [nodata() calculator]",sucker_num);
		main_nodata_loop();
	}
	else if(sucker_num == 3)
	{
/* Fourth instance of allview_suckerd periodically pings hosts */
		allview_log( LOG_LEVEL_WARNING, "server #%d started [ICMP pinger]",sucker_num);
		main_pinger_loop();
	}
	else
	{
#ifdef HAVE_SNMP
		init_snmp("allview_server");
		allview_log( LOG_LEVEL_WARNING, "server #%d started [Sucker. SNMP:ON]",sucker_num);
#else
		allview_log( LOG_LEVEL_WARNING, "server #%d started [Sucker. SNMP:OFF]",sucker_num);
#endif
