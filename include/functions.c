...
int	evaluate_COUNT_thread(MYSQL *database, char *value,DB_ITEM	*item,int parameter)
{
	...
	{
		allview_log(LOG_LEVEL_DEBUG, "Result for COUNT is empty" );
		res = FAIL;
	}
	else
	{
...
int	evaluate_SUM_thread(MYSQL *database, char *value,DB_ITEM	*item,int parameter)
{
	...
	if(DBnum_rows(result) == 0)
	{
		allview_log(LOG_LEVEL_DEBUG, "Result for SUM is empty" );
		res = FAIL;
	}
	else
	{
...
int	evaluate_AVG_thread(MYSQL *database,char *value,DB_ITEM	*item,int parameter)
{
	...
	if(DBnum_rows(result) == 0)
	{
		allview_log(LOG_LEVEL_DEBUG, "Result for AVG is empty" );
		res = FAIL;
	}
	else
	{
...
int	evaluate_MIN_thread(MYSQL *database, char *value,DB_ITEM	*item,int parameter)
{
	...
	if(DBnum_rows(result) == 0)
	{
		allview_log(LOG_LEVEL_DEBUG, "Result for MIN is empty" );
		res = FAIL;
	}
	else
	{
...
int	evaluate_DELTA_thread(MYSQL *database,char *value,DB_ITEM *item,int parameter)
{
	...
	if(DBnum_rows(result) == 0)
	{
		allview_log(LOG_LEVEL_DEBUG, "Result for DELTA is empty" );
		res = FAIL;
	}
	else
	{
...
int	evaluate_FUNCTION_thread(MYSQL *database, char *value,DB_ITEM *item,char *function,char *parameter, int flag)
{
	...
	if(strcmp(function,"last")==0)
	{
		...
		else
		{
			if(item->value_type==ITEM_VALUE_TYPE_FLOAT)
			{
				allview_log( LOG_LEVEL_DEBUG, "In evaluate_FUNCTION() 1");
				snprintf(value,MAX_STRING_LEN-1,"%f",item->lastvalue);
				del_zeroes(value);
				allview_log( LOG_LEVEL_DEBUG, "In evaluate_FUNCTION() 2");
			}
			else
			{
/*				*value=strdup(item->lastvalue_str);*/
				allview_log( LOG_LEVEL_DEBUG, "In evaluate_FUNCTION() 3 [%s] [%s]",value,item->lastvalue_str);
				strcpy(value,item->lastvalue_str);
				allview_log( LOG_LEVEL_DEBUG, "In evaluate_FUNCTION() 4");
			}
		}
	}
	...
	{
		allview_log( LOG_LEVEL_WARNING, "Unsupported function:%s",function);
		allview_syslog("Unsupported function:%s",function);
		ret = FAIL;
	}
...
/*		if(item->multiplier == 1)
		{
			allview_log( LOG_LEVEL_WARNING, "HMM [%s] [%f] [%f]",item->formula, value_float, atof(item->formula));
		...
		allview_log( LOG_LEVEL_DEBUG, "Value [%s] [%f] Suffix [%s] Units [%s]",value,value_float,suffix,item->units);
//		if(cmp_double((double)round(value_float), value_float) == 0)
	...
	allview_log( LOG_LEVEL_DEBUG, "End of evaluate_FUNCTION. Result [%s]",value);
	return ret;
}
#endif
...
int	evaluate_FUNCTION(char *value,DB_ITEM *item,char *function,char *parameter, int flag)
{
	...
	allview_log( LOG_LEVEL_DEBUG, "In evaluate_FUNCTION() Function [%s] flag [%d]",function,flag);
	...
	if(strcmp(function,"last")==0)
	{
		...
		else
		{
			if(item->value_type==ITEM_VALUE_TYPE_FLOAT)
			{
				...
				snprintf(value,MAX_STRING_LEN-1,"%f",item->lastvalue);
				del_zeroes(value);
				allview_log( LOG_LEVEL_DEBUG, "In evaluate_FUNCTION() 2 value [%s]", value);
			}
			else
			{
/*				*value=strdup(item->lastvalue_str);*/
	...
	allview_log( LOG_LEVEL_DEBUG, "In evaluate_FUNCTION() pre-7");
	allview_log( LOG_LEVEL_DEBUG, "In evaluate_FUNCTION() 7 Formula [%s]", item->formula);
	allview_log( LOG_LEVEL_DEBUG, "In evaluate_FUNCTION() 7 Value [%s]", value);
	allview_log( LOG_LEVEL_DEBUG, "In evaluate_FUNCTION() 7 Units [%s]", item->units);
	allview_log( LOG_LEVEL_DEBUG, "In evaluate_FUNCTION() 7 Value [%s] Units [%s] Formula [%s]", value, item->units, item->formula);
...
void	update_functions_thread(MYSQL *database, DB_ITEM *item)
{
	...
	allview_log( LOG_LEVEL_DEBUG, "In update_functions(%d)",item->itemid);
	...
	for(i=0;i<DBnum_rows(result);i++)
	{
		...
		allview_log( LOG_LEVEL_DEBUG, "ItemId:%d Evaluating %s(%d)\n",function.itemid,function.function,function.parameter);

		ret = evaluate_FUNCTION_thread(database,value,item,function.function,function.parameter, EVALUATE_FUNCTION_NORMAL);
		if( FAIL == ret)	
		{
			allview_log( LOG_LEVEL_DEBUG, "Evaluation failed for function:%s\n",function.function);
			continue;
		}
		allview_log( LOG_LEVEL_DEBUG, "Result of evaluate_FUNCTION [%s]\n",value);
		if (ret == SUCCEED)
		{
...
void	send_to_user_medias_thread(MYSQL *database, DB_TRIGGER *trigger,DB_ACTION *action, int userid)
{
	...
	for(i=0;i<DBnum_rows(result);i++)
	{
		...
		allview_log( LOG_LEVEL_DEBUG, "Trigger severity [%d] Media severity [%d]",trigger->priority, media.severity);
		if(((1<<trigger->priority)&media.severity)==0)
		{
			allview_log( LOG_LEVEL_DEBUG, "Won't send message");
			continue;
		}
...
void	send_to_user_thread(MYSQL *database, DB_TRIGGER *trigger,DB_ACTION *action)
{
	...
	{
		allview_log( LOG_LEVEL_WARNING, "Unknown recipient type [%d] for actionid [%d]",action->recipient,action->actionid);
		allview_syslog("Unknown recipient type [%d] for actionid [%d]",action->recipient,action->actionid);
	}
}
#endif
...
void	apply_actions_thread(MYSQL *database, DB_TRIGGER *trigger,int trigger_value)
{
	...
	allview_log( LOG_LEVEL_DEBUG, "In apply_actions(%d,%d)",trigger->triggerid, trigger_value);

	if(TRIGGER_VALUE_TRUE == trigger_value)
	{
		allview_log( LOG_LEVEL_DEBUG, "Check dependencies");
		...
		if(DBnum_rows(result) == 1)
		{
			...
			{
				allview_log( LOG_LEVEL_DEBUG, "Will not apply actions");
				DBfree_result(result);
				return;
			}
		}
	...
	allview_log( LOG_LEVEL_DEBUG, "Applying actions");
...
/*			substitute_hostname(trigger->triggerid,action.message);
		...
		{
			allview_log( LOG_LEVEL_WARNING, "Unsupported scope [%d] for actionid [%d]", action.scope, action.actionid);
			allview_syslog("Unsupported scope [%d] for actionid [%d]", action.scope, action.actionid);
		}

	}
	allview_log( LOG_LEVEL_DEBUG, "Actions applied for trigger %d %d", trigger->triggerid, trigger_value );
	DBfree_result(result);
}
#endif
...
/* Do nothing */
		...
		{
			allview_log( LOG_LEVEL_ERR, "Unknown calculation algorithm of service status [%d]", algorithm);
			allview_syslog("Unknown calculation algorithm of service status [%d]", algorithm);
		}
	}
	DBfree_result(result);
...
void	update_triggers_thread(MYSQL *database, int itemid)
{
	...
	allview_log( LOG_LEVEL_DEBUG, "In update_triggers [%d]", itemid);
...
/* Is it correct SQL? */
	...
	for(i=0;i<DBnum_rows(result);i++)
	{
		...
		{
			allview_log( LOG_LEVEL_WARNING, "Expression [%s] cannot be evaluated.",trigger.expression);
			allview_syslog("Expression [%s] cannot be evaluated.",trigger.expression);
			continue;
		}
...
/* Oprimise a little bit */
		...
		allview_log( LOG_LEVEL_DEBUG, "b trigger.value prevvalue [%d] [%d] [%d]", b, trigger.value, prevvalue);
...
int	get_lastvalue_thread(MYSQL *database, char *value,char *host,char *key,char *function,char *parameter)
{
	...
	allview_log(LOG_LEVEL_DEBUG, "In get_lastvalue()" );
	...
	{
        	DBfree_result(result);
		allview_log(LOG_LEVEL_WARNING, "Query [%s] returned empty result", sql );
		allview_syslog("Query [%s] returned empty result", sql );
		return FAIL;	
	}
	...
	allview_log(LOG_LEVEL_DEBUG, "Itemid:%d", item.itemid );

	allview_log(LOG_LEVEL_DEBUG, "Before evaluate_FUNCTION()" );
...
int	process_data(int sockfd,char *server,char *key,char *value)
{
	...
	allview_log( LOG_LEVEL_DEBUG, "In process_data()");
...
void	process_new_value_thread(MYSQL *database, DB_ITEM *item,char *value)
{
	...
	allview_log( LOG_LEVEL_DEBUG, "In process_new_value()");
	...
	if(item->history>0)
	{
		if(item->value_type==ITEM_VALUE_TYPE_FLOAT)
		{
			...
			{
				allview_log(LOG_LEVEL_ERR, "Value not stored for itemid [%d]. Unknown delta [%d]", item->itemid, item->delta);
				allview_syslog("Value not stored for itemid [%d]. Unknown delta [%d]", item->itemid, item->delta);
				return;
			}
		}
		else
